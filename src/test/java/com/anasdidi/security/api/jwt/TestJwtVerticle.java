...
public class TestJwtVerticle {
  ...
  void testJwtRefreshSuccess(Vertx vertx, VertxTestContext testContext) {
    webClient.post(port, host, requestURI + "/login").rxSendJsonObject(user)
        .subscribe(response1 -> {
          ...
          Thread.sleep(1000);
          webClient.post(port, host, requestURI + "/refresh")
  ...
  @Test
  void testJwtRefreshValidationError(Vertx vertx, VertxTestContext testContext) {
    webClient.post(port, host, requestURI + "/login").rxSendJsonObject(user)
        .subscribe(response1 -> {
          JsonObject data1 = response1.bodyAsJsonObject().getJsonObject("data");
          String accessToken = data1.getString("accessToken");
  ...
  void testJwtRefreshRequestBodyEmptyError(Vertx vertx, VertxTestContext testContext) {
    webClient.post(port, host, requestURI + "/login").rxSendJsonObject(user)
        .subscribe(response1 -> {
          ...
          webClient.post(port, host, requestURI + "/refresh")
              .putHeader("Authorization", "Bearer " + accessToken).rxSend().subscribe(response2 -> {
                testContext.verify(() -> {
                  Assertions.assertEquals(400, response2.statusCode());
                  Assertions.assertEquals("application/json", response2.getHeader("Content-Type"));
                  Assertions.assertEquals("no-store, no-cache",
