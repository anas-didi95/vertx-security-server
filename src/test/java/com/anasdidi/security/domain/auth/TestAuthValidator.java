...
public class TestAuthValidator {
  ...
  @Test
  void testValidateCheck(Vertx vertx, VertxTestContext testContext) {
    List<String> actualList = new AuthValidator().validateCheck(AuthVO.fromJson(new JsonObject()));
    List<String> expectedList =
        Arrays.asList("User Id not defined in token!", "Permissions not defined in token!");
  ...
  @Test
  void testValidateRefresh(Vertx vertx, VertxTestContext testContext) {
    List<String> actualList =
        new AuthValidator().validateRefresh(AuthVO.fromJson(new JsonObject()));
    List<String> expectedList = Arrays.asList("Token Id not defined in token!");
